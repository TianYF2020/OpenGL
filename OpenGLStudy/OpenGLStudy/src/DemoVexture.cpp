...
{
	glfwInit();
	int screenWidth, screenHeight, imgWidth, imgHeight;
	unsigned char* image = SOIL_load_image("./image/color.png", &imgWidth, &imgHeight, 0, SOIL_LOAD_RGBA);			// 获取图片
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices_Vexture), vertices_Vexture, GL_STATIC_DRAW);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices_Vexture), indices_Vexture, GL_STATIC_DRAW);
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, imgWidth, imgHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);			// 读取图片信息
